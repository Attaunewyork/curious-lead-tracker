...
export default function Dashboard() {
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total de Leads" value={leads.length} icon={Users} trend={{
        value: 12,
        isPositive: true
      }} />
        <StatCard title="Valor Total" value={formatCurrency(totalValue)} icon={DollarSign} trend={{
        value: 8,
        isPositive: true
      }} />
        <StatCard title="Taxa de Conversão" value={`${conversionRate}%`} icon={Target} trend={{
        value: 5,
        isPositive: true
      }} />
        <StatCard title="Leads Ativos" value={leads.length - statusStats['closed-won'] - statusStats['closed-lost']} icon={TrendingUp} trend={{
        value: 3,
        isPositive: false
      }} />
      </div>
              ...
              {Object.entries(statusStats).map(([status, count]) => <div key={status} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-32 bg-muted rounded-full h-2">
                      <div className="bg-gradient-to-r from-[#FF8360] to-[#FF3C7E] h-2 rounded-full transition-all duration-300" style={{
        ...
        <Card className="bg-zinc-950">
          ...
          <CardContent>
            <div className="space-y-3">
              {leads.slice(0, 5).map(lead => <div key={lead.id} className="flex items-center justify-between p-3 bg-muted rounded-lg border">
                  <div>
